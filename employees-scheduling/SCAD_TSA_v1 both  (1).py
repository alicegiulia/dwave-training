...
def staff_schedule_simulated(n_employees, dataDir):
...
def staff_schedule_quantum(n_employees, dataDir):
    ...
    sampler = LeapHybridSampler(token='DEV-6428b06340440c170283af70c15c888faf277261')
...
#get simulation
for n in range(10, tot_employees, 100):

    staff_sim = staff_schedule_simulated(n, dataDir)
    n_employees_sim.append(staff_sim[1])
    TTS_sim.append(staff_sim[0])

#get quantum     
for n in range(10, tot_employees, 100):

    staff_quantum = staff_schedule_quantum(n, dataDir)
    n_employees_qu.append(staff_quantum[1])
    TTS_qu.append(staff_quantum[0])


#get plots    
plt.plot(n_employees_sim, TTS_sim, color = 'red', label = 'simulated')
plt.ylabel('TTS [s]')
plt.xlabel('Number of employees staffed')

plt.plot(n_employees_qu, TTS_qu, color = 'blue', label = 'quantum')

plt.title('Quantum vs simulated annealing- staff schedule')

plt.legend()

plt.show()

plt.savefig('QuantumVSsimulated_scaling.png')
