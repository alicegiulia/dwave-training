...
dataDir = r'C:\Users\scerto\Quantum\QuantumPython\SCAD_TSA'

employeeDF = pd.read_excel(dataDir + r'\SCAD_TSA_simulated_data_final_v3.xlsx', sheet_name = 'Employee')
shiftDF = pd.read_excel(dataDir + r'\SCAD_TSA_simulated_data_final_v3.xlsx', sheet_name = 'Shift_details')
...
sampler = LeapHybridSampler(token='eUXo-59c6b9bcf50e9d75f9550734efcc4143ac42c956')


bqm = 2*obj_func + const_1 + const_2 + const_3 + 3*const_4 + const_5
...
outputDF.to_excel(dataDir + r'\BWI_Gender1_output.xlsx')


# #For some reason variable labels sometimes dont get returned
# employeeShiftAssignment = pd.DataFrame(columns = ['empid', 'rank', 'shift_day','calendar_date'])


# for i, kv in enumerate(list(sample.items())):
#     k = bqm.variables[i]
#     if k[:1] != 'E':
#         continue
#     if kv[1] != 1:
