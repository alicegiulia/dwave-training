...
def get_graph():
    ...
    # Change this parameter to change the size of the graph
    graph_size = 2000
...
def get_bqm(G):
    ...
    for i, j in G.edges:
        ...
        #Q[(i,i)]= 1*rand
        bqm.add_variable(i, 1*rand)
        #Q[(j,j)]= 1*rand
        bqm.add_variable(j, 1*rand)
        #Q[(i,j)]= -2*rand
        bqm.add_interaction(i,j, -2*rand)
...
# Add the hybrid sampler and return the sampleset
